...
export const UploadMarksModal = () => {
  ...
  const handleUpload = () => {
    if (!file) {
      addToast('No file selected', 'Please select an Excel file containing examination marks.', 'warning');
      return;
    }
    ...
    setTimeout(() => {
      ...
      addToast('Marks Uploaded', `Successfully imported examination marks for 120 students from "${file.name}".`, 'success');
      setActiveModal(null);
      setFile(null);
    }, 1200);
  };
  ...
  return (
    <Modal
      isOpen={activeModal === 'uploadMarks'}
      onClose={() => setActiveModal(null)}
      title="Upload Examination Marks (Excel / CSV)"
    >
      <div className="space-y-4">
        <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl p-8 text-center hover:border-primary-500 transition-colors">
          <FileSpreadsheet className="w-10 h-10 mx-auto text-primary-500 mb-2" />
          <p className="text-sm font-bold text-slate-700 dark:text-slate-200">Upload Examination Marksheet</p>
          <p className="text-xs text-slate-400 mt-1">Columns required: RollNo, Internal, Assignment, Practical, EndSem</p>
        ...
        {file && (
          <div className="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-xl text-xs">
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-700">
          ...
          >
            {isUploading ? 'Validating Scores...' : 'Upload & Auto-Calculate Grades'}
          </button>
        </div>
      </div>
    </Modal>
